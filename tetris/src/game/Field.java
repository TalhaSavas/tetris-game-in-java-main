...
public class Field {
	...
	public void render(Graphics g) {
		...
		g.drawString("Score: " + getScore(), 340, 450);
	...
	public boolean isPiecePlaceable(Piece currentPiece) {
		int pieceX = currentPiece.getX();
		int pieceY = currentPiece.getY();

		PieceShape pieceShape = currentPiece.getShape();
		int pieceHeight = pieceShape.getHeight();
		int pieceWidth = pieceShape.getWidth();

		for (int row = 0; row < pieceHeight; row++) {
			for (int col = 0; col < pieceWidth; col++) {
				if (pieceShape.isPieceBrick(row, col) && this.checkIsFieldBrick(pieceY + row, pieceX + col)) {
					return false;
				}
			}
		}
		...
		for (int row = 0; row < pieceHeight; row++) {
			for (int col = 0; col < pieceWidth; col++) {
				if (pieceShape.isPieceBrick(row, col)) {
					...
					this.fieldColor[pieceY + row][pieceX + col] = FIELD_COLORS[this.random.nextInt(FIELD_COLORS.length)];
				}
			}
		}
	}
	...
	public void destroyFullRows() {
		...
		this.score += fullRows * 10;
	}

	public boolean isPieceOut(Piece currentPiece) {
		int pieceX = currentPiece.getX();
		int pieceY = currentPiece.getY();
		...
		for (int row = 0; row < pieceHeight; row++) {
			for (int col = 0; col < pieceWidth; col++) {
				if (pieceShape.isPieceBrick(row, col) && (pieceY + row < 0 || pieceY + row >= this.getHeight())) {
					return true;
				}
			}
		}
	...
	public boolean doesPieceTouchRightWall(Piece currentPiece) {
		int pieceX = currentPiece.getX();
		int pieceY = currentPiece.getY();
		...
		for (int row = 0; row < pieceHeight; row++) {
			for (int col = 0; col < pieceWidth; col++) {
				if (pieceShape.isPieceBrick(row, col) && (pieceX + col + 1 >= this.getWidth())) {
					return true;
				}
			}
		}
	...
	public boolean doesPieceTouchesBottom(Piece currentPiece) {
		int pieceX = currentPiece.getX();
		int pieceY = currentPiece.getY();
		...
		for (int row = 0; row < pieceHeight; row++) {
			for (int col = 0; col < pieceWidth; col++) {
				if (pieceShape.isPieceBrick(row, col) && (pieceY + row + 1 >= this.getHeight())) {
					return true;
				}
			}
		}
